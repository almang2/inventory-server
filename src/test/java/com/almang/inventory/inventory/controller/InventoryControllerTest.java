...
public class InventoryControllerTest {
        ...
        private UsernamePasswordAuthenticationToken auth() {
                CustomUserPrincipal principal = new CustomUserPrincipal(1L, "inventory_admin", List.of());
        ...
        void 재고_수동_수정시_재고가_존재하지_않으면_예외가_발생한다() throws Exception {
                ...
                mockMvc.perform(patch("/api/v1/inventory/{inventoryId}", inventoryId)
                                ...
                                .andExpect(jsonPath("$.status")
                                                .value(ErrorCode.INVENTORY_NOT_FOUND.getHttpStatus().value()))
        ...
        void 상점_재고_목록_조회에_성공한다() throws Exception {
                ...
                mockMvc.perform(get("/api/v1/inventory")
                                ...
                                .andExpect(jsonPath("$.status").value(200))
                                .andExpect(jsonPath("$.message")
                                                .value(SuccessMessage.GET_STORE_INVENTORY_SUCCESS.getMessage()))
                                ...
                                .andExpect(jsonPath("$.data.content[0].cafe24Code").value("C001"))
                                .andExpect(jsonPath("$.data.content[0].posCode").value("P001"));
        }
        ...
        void 재고_이동에_성공한다() throws Exception {
                ...
                mockMvc.perform(post("/api/v1/inventory/{inventoryId}/move", inventoryId)
                                ...
                                .andExpect(jsonPath("$.status").value(200))
                                .andExpect(jsonPath("$.message")
                                                .value(SuccessMessage.MOVE_INVENTORY_SUCCESS.getMessage()))
                                .andExpect(jsonPath("$.data.inventoryId").value(inventoryId))
                                .andExpect(jsonPath("$.data.productId").value(productId))
                                .andExpect(jsonPath("$.data.productName").value("상품1"))
                                .andExpect(jsonPath("$.data.cafe24Code").value("C001"))
                                .andExpect(jsonPath("$.data.posCode").value("P001"))
        ...
        void 재고_이동시_재고_도메인_예외가_발생하면_에러_응답을_반환한다() throws Exception {
                ...
                mockMvc.perform(post("/api/v1/inventory/{inventoryId}/move", inventoryId)
                                ...
                                .andExpect(status().isBadRequest())
                                .andExpect(jsonPath("$.status")
                                                .value(ErrorCode.DISPLAY_STOCK_NOT_ENOUGH.getHttpStatus().value()))
